...
class Rule {
    ...
    void addDependency(Rule dependent) {
        _depends.addEdgeWithCount(this.getVertex(), dependent.getVertex());
    }
    ...
    void addCommands(List<String> commands) {
        if (commands.size() > 0 && _commands.size() > 0) {
            throw new IllegalStateException("Error: too much commands");
        }
    ...
    private void checkFinishedDependencies() {
        ...
        for (int i : _depends.successors(getVertex())) {
            if (_depends.getLabel(i).isUnfinished()) {
                error("Error: Dependencies not yet finished");
            }
        }
    }
    ...
    private boolean outOfDate() {
        ...
        for (int i : _depends.successors(getVertex())) {
            if (_depends.getLabel(i).getTime() > this.getTime()) {
                return true;
            }
        }
        return false;
