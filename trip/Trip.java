...
class Trip {
    ...
    int reportSegment(int seq, int from, List<Integer> segment) {
        ...
        ArrayList<Road> massaged = new ArrayList<>();

        Road lastRoad = _map.getLabel(segment.get(0), segment.get(1));
        massaged.add(lastRoad);
        for (int i = 1; i < segment.size() - 1; i++) {
            Road curRoad =
                    _map.getLabel(segment.get(i), segment.get(i + 1));
            if (curRoad.toString().equals(lastRoad.toString())
                    && curRoad.direction() == lastRoad.direction()) {
                Road combined = new Road(curRoad.toString(),
                        curRoad.direction(),
                        curRoad.length() + lastRoad.length());
                massaged.remove(lastRoad);
                massaged.add(combined);
                lastRoad = combined;
            } else {
                massaged.add(curRoad);
                lastRoad = curRoad;
            }
        }

        for (int i = 0; i < massaged.size() - 1; i++) {
            System.out.printf("%d. Take %s %s for %.1f miles.",
                    i + seq,  massaged.get(i),
                    massaged.get(i).direction().fullName(),
                    massaged.get(i).length());
            System.out.println();
        }
        System.out.printf("%d. Take %s %s for %.1f miles to %s.",
                massaged.size() + seq - 1,
                massaged.get(massaged.size() - 1),
                massaged.get(massaged.size() - 1).direction().fullName(),
                massaged.get(massaged.size() - 1).length(),
                _map.getLabel(segment.get(segment.size() - 1)));
        System.out.println();
        return seq + massaged.size();
    }
    ...
    private class TripPlan extends SimpleShortestPaths {
        ...
        protected double estimatedDistance(int v) {
            Location destObj = _map.getLabel(getDest());
            Location vObj = _map.getLabel(v);
            return destObj.dist(vObj);
        }
